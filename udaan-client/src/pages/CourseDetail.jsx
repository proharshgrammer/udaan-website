         ...
         <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-8 flex flex-col md:flex-row items-stretch">
            ...
            <div className="w-full md:w-[45%] shrink-0 bg-gray-50 relative min-h-[250px] md:min-h-full flex items-center justify-center p-4 sm:p-6 lg:p-8 border-t md:border-t-0 md:border-l border-gray-100">
               {((course.thumbnail || course.thumbnailUrl) && !(course.thumbnail || course.thumbnailUrl).includes('cloudinary.com')) || getHardcodedCourseImage(course.name) ? (
                  <img src={getHardcodedCourseImage(course.name) || course.thumbnail || course.thumbnailUrl} alt={course.name} className="w-full h-full object-contain rounded-xl mix-blend-multiply" />
               ) : (
