...
export default function News() {
  ...
  const publicNotices = notices?.filter(n => n.sentTo === 'all') || [];

  const renderNotice = (n) => (
    <div key={n.id} className="bg-white rounded-2xl p-7 border border-gray-100 shadow-sm mb-6 group transition hover:shadow-md">
      {/* Title (if provided) */}
      {n.title && (
        <div className="flex justify-between items-start gap-4 mb-3">
          <h3 className="font-heading font-semibold text-lg md:text-xl pr-6 text-gray-900">
             {n.title}
          </h3>
        </div>
      )}
      ...
      {/* Message content */}
      <p className="font-body text-[15px] text-gray-600 mb-5 leading-[1.7] whitespace-pre-wrap">{n.message}</p>
      
      {/* Image Attachment */}
      {n.type === 'image' && n.imageUrl && (
        <div className="mb-6 rounded-xl overflow-hidden border border-gray-100/50 bg-gray-50 flex items-center justify-center">
          <img src={n.imageUrl} alt="Announcement Attachment" className="max-h-[400px] object-contain w-full" />
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mt-auto pt-5 border-t border-gray-100/60">
        <div className="flex gap-2 flex-wrap items-center">
           <span className="text-[13px] text-gray-400 font-medium flex items-center gap-1">
             <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
             {n.timestamp?.toDate().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
           </span>
        </div>
        {n.link && (
          <a href={n.link} target="_blank" rel="noopener noreferrer" className="text-brand-blue text-[13px] font-semibold hover:underline inline-flex items-center gap-1 shrink-0 bg-blue-50/50 px-3 py-1.5 rounded-full">
            Open Link
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Helmet>
        <title>Latest Announcements — Udaan Vidyapeeth</title>
        <meta name="description" content="Stay updated with the latest news and important dates." />
      </Helmet>
      <Navbar />
        ...
        {loading ? (
          ...
          <>
            <div className="flex flex-col gap-2">
              {publicNotices.map(n => renderNotice(n))}
              {publicNotices.length === 0 && (
                <div className="text-center py-20 font-body bg-brand-light/10 rounded-2xl border border-brand-light/30 shadow-sm flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-lg font-medium text-brand-dark">No announcements at the moment. Check back soon.</p>
                </div>
              )}
            </div>
          </>
