...
export default function Checkout() {
  ...
  // Student details form — field names match app's UserModel
      ...
      try {
        ...
        if (profileSnap.exists()) {
          ...
          setForm((prev) => ({
            ...
            state: p.state || p.homeState || "",
            field: p.field || p.category || "",
            rank: p.rank || p.crlRank || "",
  ...
  const handleApplyCoupon = () => {
    ...
    // Check expiry
    const expiry = match.expiryDate?.toDate
      ? match.expiryDate.toDate()
      : new Date(
          match.expiryDate?.seconds
            ? match.expiryDate.seconds * 1000
            : match.expiryDate,
        );
    if (expiry < new Date()) {
  ...
  if (appliedCoupon) {
    const discountAmount =
      (finalPrice * Number(appliedCoupon.discountPercentage)) / 100;
  ...
  const handlePay = async (e) => {
    ...
    setProcessing(true);
    try {
      // 1. Save/update user profile — field names match app's UserModel exactly
      const currentDoc = await getDoc(doc(db, COLLECTIONS.USERS, currentUser.uid));
      const existing = currentDoc.exists() ? currentDoc.data() : {};
      ...
      await setDoc(
        doc(db, COLLECTIONS.USERS, currentUser.uid),
        {
          ...
          field: form.field,
          rank: form.rank,
          // Preserve existing values for fields not collected at checkout
          dob: existing.dob || "",
          exam: existing.exam || "",
          interest: existing.interest || "",
          profileImageUrl: existing.profileImageUrl || "",
          // Extra counseling fields (not in app model, but useful data)
      ...
      if (!window.Razorpay)
        throw new Error("Payment SDK not loaded. Please reload.");
          ...
          try {
            ...
            await setDoc(
              ...
              ),
              {
                ...
                // Extra fields for web reference (app only reads studentId + enrolledDate)
                name: form.name,
                email: form.email,
                phoneNumber: form.phoneNumber,
                state: form.state,
            ...
            alert(
              "Payment successful, but error updating dashboard. Support will sync this shortly.",
            );
            navigate("/my-courses");
          }
        },
      ...
      console.error(err);
      alert(
        "Error initiating payment: " + (err.message || "Please try again."),
      );
    } finally {
      setProcessing(false);
    }
    ...
    <div className="bg-gray-50 min-h-screen font-body flex flex-col">
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-8 lg:py-12 mt-20">
        ...
        <div className="mb-8">
          <button
            onClick={() => navigate(-1)}
            className="text-gray-500 hover:text-brand-blue font-medium text-sm flex items-center gap-1 mb-4 transition"
          >
            <svg
              className="w-4 h-4"
              fill="none"
            ...
            <form onSubmit={handlePay} id="checkout-form">
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                      CRL Rank <span className="text-red-400">*</span>
                    </label>
                    <div className="relative">
                      <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400">
                        <svg
                ...
                {appliedCoupon && (
                  <div className="flex justify-between items-center text-sm mb-3 text-green-600">
                    ...
                    <span className="font-semibold">
                      - ₹
                      {Math.floor(
                        coursePrice * (appliedCoupon.discountPercentage / 100),
